...
export default function FAQWithSpiral() {
  ...
  // FAQ content customized for Tattvadrishti
  const faqs = [
    {
      q: "What is Tattvadrishti and what does it do?",
      a: "Tattvadrishti is an advanced AI-powered content moderation and security intelligence platform. We provide real-time detection of harmful content, watermarking technology, federated blockchain tracking, and comprehensive analytics for digital content safety.",
    },
    {
      q: "How does the AI detection work?",
      a: "Our platform integrates Azure OpenAI and Content Safety APIs to analyze text, images, and documents in real-time. We detect hate speech, violence, self-harm, sexual content, and malicious activity with configurable severity thresholds and detailed confidence scores.",
    },
    {
      q: "What technologies power Tattvadrishti?",
        ...
        <header className="mb-10 flex items-end justify-between border-b border-white/20 pb-6">
          <div>
            ...
            <p className="mt-2 text-sm md:text-base text-white/70">
              Everything you need to know about Tattvadrishti
            </p>
          </div>
          <div className="flex items-center gap-3">
            <input
              ...
              className="h-10 w-56 rounded-xl border border-white/20 bg-transparent px-3 text-sm outline-none transition focus:border-emerald-500/60"
            />
          </div>
        </header>
        ...
        <aside className="fixed right-4 top-4 z-20 w-[320px] rounded-2xl border border-white/15 bg-black/70 p-4 backdrop-blur">
          ...
          <div className="space-y-3 text-xs">
            ...
            <div className="flex gap-2">
              <button
                onClick={randomize}
                className="w-full rounded-xl border border-white/20 px-3 py-2 text-xs hover:border-emerald-500/50 transition-colors"
              >
                Randomize (R)
              </button>
              <button
                onClick={() => setPanelOpen(false)}
                className="rounded-xl border border-white/20 px-3 py-2 text-xs hover:border-emerald-500/50 transition-colors"
              >
                Close (H)
              </button>
            </div>
  ...
  return (
    <label className="flex items-center justify-between">
      ...
      <button
        ...
        className={`h-6 w-10 rounded-full border border-white/20 transition ${value ? "bg-emerald-500" : "bg-transparent"}`}
        aria-pressed={value}
      >
        <span className={`block h-5 w-5 translate-x-0.5 rounded-full bg-white transition ${value ? "translate-x-4" : "translate-x-0"}`} />
      </button>
    </label>
  );
}
